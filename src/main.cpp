...
#if defined(NDEBUG)
# error "Shibagold cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x03391f8fc2db68d4533ba422721ac6b8db0aa5c27bd5adc45945696260eac99c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Shibagold: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Shibagold Signed Message:\n";
...
{
    // Shibagold: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1576800); // Shibagold: 1576800 blocks in ~6 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 60 * 60; // Shibagold: 4 hours
static const int64 nTargetSpacing = 10; // Shibagold: 10 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Shibagold: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Shibagold: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x31f00acd151babbc01aca170ad6f67f84eb22e4cc9f863fe982358c06c9a2182");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ShibaGold was launched!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 32 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044c844c37f5541d9c5a632ea5a9f1ba429d54de953545b64a1cc23cd05b1a6adb7b2cb4bb93bea1e32650214a8a5fe1c61977ee62d93f752a3df88584343c26d0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1636299152;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084763880;

        if (fTestNet)
        {
            block.nTime    = 1636299084;
            block.nNonce   = 385836918;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe0ba44dbcdabf305febc4bfbff51cce146e239677a9464318e507fd1d536c5dd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb2, 0xdd }; // Shibagold
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ShibagoldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ShibagoldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ShibagoldMiner(CWallet *pwallet)
{
    printf("ShibagoldMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("shibagold-miner");
    ...
    try { loop {
        ...
        printf("Running ShibagoldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ShibagoldMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ShibagoldMiner, pwallet));
}
